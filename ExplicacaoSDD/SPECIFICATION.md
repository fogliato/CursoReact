...
### Session 2026-07-24
- Q: Onde os dados de especialidade devem persistir após envio do formulário? → A: localStorage com sincronização automática ao estado do componente pai
- Q: O que deve acontecer quando o formulário é resetado? → A: Limpar completamente tanto o formulário quanto o localStorage (limpeza total)
- Q: Quando a mensagem de erro deve desaparecer? → A: Mensagem persiste até que o campo contenha entrada válida
- Q: O comportamento de validação muda após múltiplas tentativas de envio? → A: Comportamento permanece consistente em todos os envios (sem escalonamento ou mudanças)
- Q: Como o card deve exibir quando o dados de especialidade estão ausentes? → A: Omitir completamente a linha de especialidade se a propriedade estiver faltando
